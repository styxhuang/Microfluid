...
N_cell = 1 #number of cells, only 0 or 1
lz = 20.0
box_len = lx = ly = 15.0
mass = 0.10
...
#Ming function
def atomlist(vrtList,tagnum):
    Atom_list={}
    for i in range(len(vrtList)):
        Atom_list.setdefault(i+tagnum,[]).append(vrtList[i])
        #Atom_index.append((vrtList[i]))
    keys=Atom_list.keys()
    return keys,Atom_list

#Ming function
def anglebonds(alist,r,modi): #generate angle bonds
    bonds=[]
    ...
    for i in range(len(alist)):
        ...
        for j in range(len(alist)):
            ...
            atom2=alist[jj]
            for k in range(3):
                conditions=+(np.array(atom1[0][k])**2-np.array(atom2[0][k])**2)               
            if  atom1 != atom2: 
                if np.sqrt(abs(conditions)) < r:
                    for q in range(len(alist)):
                        qq=q+modi
                        atom3=alist[qq]
                        for k in range(3):
                            conditions=+(np.array(atom2[0][k])**2-np.array(atom3[0][k])**2)
                        if  atom2 != atom3:
                            if atom1 != atom3: 
                                if np.sqrt(abs(conditions)) < r:
                                    atomindex=[keys[i],keys[j],keys[q]]
                                    bonds.append(atomindex)
                    
    return bonds

#Ming function
def dihbonds(alist,r,modi): #generate angle bonds
    bonds=[]
    keys=list(alist.keys())
    for i in range(len(alist)):
        atomindex=[]
                            ...
                            if atom1 != atom3:
                                if np.sqrt(abs(conditions)) < r:
                                    for q in range(len(alist)):
                                        ...
                                        atom4=alist[qq]
                                        for k in range(3):
                                            conditions=+(np.array(atom3[0][k])**2-np.array(atom4[0][k])**2)
                                        if  atom1 != atom4:
                                            if atom2 != atom4:
                                                if atom3 != atom4: 
                                                    if np.sqrt(abs(conditions)) < r:
                                                        atomindex=[keys[i],keys[j],keys[p],keys[q]]
                                                        bonds.append(atomindex)
                                                        #print "atom=",i,j,p,q,"finish"
...
def rm_repeat(alist):
    alist.sort()
    f_list=list(alist for alist,_ in itertools.groupby(alist))
    return f_list
...
for m in range(len(N_cell*micelle)):
...
cnt = 0
mcl_z_arr_idx = 0
mcl_z_arr = np.linspace(-1*system.box.Lz/4.75,system.box.Lz/4.75,N_cell)
...
# Ming code for angle and dihedral bonds
atomindex=tags_mcls[0]
(keys,alist)=atomlist(micelle,atomindex)
Anglebond=rm_repeat(anglebonds(alist,r_max,atomindex))
Dihbond=rm_repeat(dihbonds(alist,r_max,atomindex))
for i in range(len(Anglebond)):
    system.angles.add("Hb",Anglebond[i][0],Anglebond[i][1],Anglebond[i][2])
for k in range(len(Dihbond)):
    system.dihedrals.add("Db",Dihbond[k][0],Dihbond[k][1],Dihbond[k][2],Dihbond[k][3])
