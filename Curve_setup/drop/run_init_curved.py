...
r_drop = 4.0 #radius of droplet
cyl_dr = 20. #width of the concentric cyls
R_in = 25.0 #set the inner R_cyl
lz = 20.0
...
drp_scl_fct = R_in# + cyl_dr/10.

# box dims
lx = ly = 2*R_out

# solve for the N_solvent from given volume fraction
N_solv = 6*solv_vf*cyl_dr**2*lz
...
def cyl_wall_builder(R,a):
    ...
    zs = np.linspace(-lz/2.,lz/2.,m.ceil(lz/a))
    cnt_z = 0
    for rad in rads:
        cnt_th = 0
...
# wall params
cyl_r_in = R_in #system.box.Lx*r_in_fctr
cyl_r_out = R_out #system.box.Lx*r_out_fctr
...
for p in system.particles:
    # test if this particle is a candidate
    xDsq = (p.position[0]+drp_scl_fct)**2
    yDsq = (p.position[1]+drp_scl_fct)**2
    zDsq = (p.position[2])**2
    r_diff_pos = (xDsq+yDsq+zDsq)**(1./2.)
    if r_diff_pos <= r_drop:# + drp_scl_fct:
